...
void ILI9488_Init()
{
    ...
    // CS pin
    tftCS_GPIO = LCD_CS_GPIO_Port;
    tftCS_PIN = LCD_CS_Pin;
    // DC pin
    tftDC_GPIO = dcPORT;
    tftDC_PIN = dcPIN;
    // HAL_GPIO_WritePin(tftCS_GPIO, tftCS_PIN, GPIO_PIN_SET);
...
void fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    ...
    // 2. Set Window (Note the -1 for end coordinates)
    setAddrWindow(x, y, x + w - 1, y + h - 1);

    // 3. Prepare 18-bit color
    uint8_t r = ((color & 0xF800) >> 11) << 3;
    uint8_t g = ((color & 0x07E0) >> 5)  << 2;
    uint8_t b = (color & 0x001F) << 3;

    // 4. Use a small, safe static buffer to prevent stack crash
    static uint8_t line_buf[320 * 3]; // One full row max
    for(int i = 0; i < w; i++) {
        line_buf[i*3] = r; line_buf[i*3+1] = g; line_buf[i*3+2] = b;
    }

    DC_DATA();
    CS_A();
    
    // 5. Transmit line by line
    for(int row = 0; row < h; row++) {
        HAL_SPI_Transmit(&hspi1, line_buf, w * 3, HAL_MAX_DELAY);
    }
...
void drawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size)
{
    ...
    // 2. Prepare the 18-bit colors (to avoid calculating inside the loop)
    uint8_t r_f = ((color & 0xF800) >> 11) << 3;
    uint8_t g_f = ((color & 0x07E0) >> 5)  << 2;
    uint8_t b_f = (color & 0x001F) << 3;

    uint8_t r_b = ((bg & 0xF800) >> 11) << 3;
    uint8_t g_b = ((bg & 0x07E0) >> 5)  << 2;
    uint8_t b_b = (bg & 0x001F) << 3;

    // 3. Define the character window (6 pixels wide, 8 pixels high)
    uint16_t char_w = 6 * size;
    uint16_t char_h = 8 * size;
    setAddrWindow(x, y, x + char_w - 1, y + char_h - 1);

    // 4. Buffer for one character (6*8 pixels * 3 bytes = 144 bytes per char at size 1)
    // For size 2, it's 576 bytes. This is safe for the stack.
    uint8_t char_buf[char_w * char_h * 3];
    uint32_t ptr = 0;

    for (int8_t j = 0; j < 8; j++) {            // For each row
        for (int8_t sy = 0; sy < size; sy++) {  // Upscaling height
            for (int8_t i = 0; i < 6; i++) {    // For each column
                uint8_t line = (i == 5) ? 0x0 : font1[(c * 5) + i];
                bool pixel_on = (line >> j) & 0x1;
                
                for (int8_t sx = 0; sx < size; sx++) { // Upscaling width
                    if (pixel_on) {
                        char_buf[ptr++] = r_f; char_buf[ptr++] = g_f; char_buf[ptr++] = b_f;
                    } else {
                        char_buf[ptr++] = r_b; char_buf[ptr++] = g_b; char_buf[ptr++] = b_b;
                    }
                }
            }
        }
    ...
    // 5. Send the entire character in ONE SPI burst
    ILI9488_SendData_Multi(char_buf, ptr);
}
